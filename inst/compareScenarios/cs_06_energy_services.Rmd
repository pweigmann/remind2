...
```{r FE Pass mix by mode w/ bunkers}
...
```{r FE Pass mix by mode w/ bunkers - per capita}
tot <- "FE|Transport|Pass with bunkers pCap"
items <- c(
...
tot <- "ES|Transport|Pass with bunkers"
items <- c(
...
```{r ES Pass by mode w/ bunkers - Bar}
tot <- "ES|Transport|Pass with bunkers pCap"
items <- c(
  "ES|Transport|Bunkers|Pass|International Aviation pCap",
  "ES|Transport|Pass|Domestic Aviation pCap",
  "ES|Transport|Pass|Rail pCap",
...
```{r ES Pass share mix by mode w/ bunkers - Bar }
...
```{r ES Pass share mix by mode w/o bunkers }
showAreaAndBarPlots(data, items, tot, orderVars = "user", fill = TRUE)
```
