...
!Attention! In the current form it only works if csN is called with 02_portal_data.Rmd being the only module in use.
Don't combine 02 with other .Rmd modules.
...
data_nz <- data %>%
  ...
  rbind(data %>%
          select(-varplus) %>%
          filter(region == regions, model %in% models, period %in% yearsCDR, scenario %in% scenarios) %>%
  ...
  quitte::calc_addVariable(
        ...
        "`Final Energy|Buildings|Non Electricity`" = "`Final Energy|Residential and Commercial` - 
        `Final Energy|Buildings|Electricity`",
...
# manually calculate low carbon electricity investments for REMIND
low_carbon_vars <- c("Investment|Energy Supply|Electricity|Nuclear",
...
data_tr2 <- data %>%
  ...
  filter(model == "REMIND-MAgPIE 3.0-4.4",
         ...
         period %in% years,
         scenario %in% scenarios,
         variable %in% low_carbon_vars) %>%
  quitte::calc_addVariable(
    "`Low-carbon electricity investments`" = c("1e-3*(`Investment|Energy Supply|Electricity|Nuclear` +
                                                `Investment|Energy Supply|Electricity|Biomass` +
                                                `Investment|Energy Supply|Electricity|Solar` +
                                                `Investment|Energy Supply|Electricity|Wind` +
                                                `Investment|Energy Supply|Electricity|Hydro` +
                                                ...
                                                `Investment|Energy Supply|Electricity|Transmission and Distribution`)",
                                               "trillion US$")) %>%
  filter(variable == "Low-carbon electricity investments")

data_tr <- rbind(data_tr, data_tr2)
...
write.table(data_macro, "macro-economic_variables_2022-28-09.csv", sep = ";", row.names = FALSE)
```
