...
# NGFS specific
library(vroom)
source("C://Users/pascalwe/Code/NGFS/remind2/inst/markdown/compareScenariosNGFS/plots_ngfs.R")
```
...
#"C://Users/pascalwe/data/NGFS/phase3/1648719942696-snapshot_ngfs_internal/output/snapshot_all_regions.csv"
# snapshot_glo  <- "C://Users/pascalwe/data/NGFS/phase3/1648719942696-snapshot_ngfs_internal/output/snapshot_world.csv"

tmp <- vroom(snapshot_regi)
# inserted from Jeromes script
data_ngfs <- tmp[, c("Model", "Scenario", "Region", "Variable", "Unit", paste(c(1995:2019, seq(2020,2100,5))))] %>% 
...
data_ngfs_p2 <- tmp_p2[, c("Model", "Scenario", "Region", "Variable", "Unit", paste(c(1990, 1995, 2000:2019, seq(2020,2100,5))))] %>% 
...
levels(dataHist$region) <- c(levels(dataHist$region), "EU", "CHN", "RUS")
dataHist[dataHist$region == "EUR", "region"] <- "EU"   # not perfect match
dataHist[dataHist$region == "CHA", "region"] <- "CHN"  # not perfect match
dataHist[dataHist$region == "REF", "region"] <- "RUS"  # not perfect match at all

# Combine into one data frame and remove old.
data_ngfs <- bind_rows(data_ngfs, data_ngfs_p2)
data <- bind_rows(data_ngfs, dataHist)
rm(data_ngfs, data_ngfs_p2, dataHist)
...
# GDP available in $2005 and $2010 in REMIND, remove 2005 (also removes historical data!)
