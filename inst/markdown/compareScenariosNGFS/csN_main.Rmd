...
if (5 %in% params$sections) {
  u_ngfs_scenario <- c("Net Zero 2050")
  tmstmp <- Sys.Date()
  
  # TODO phase 3 data, in this current form still produces errors (object 'Secondary Energy|Electricity|Wind' not found)
  # use "world" file from snapshot, do data preparation as Jerome did
#   snapshot_world <- paste0(params$snapshot, "/output/snapshot_world.csv")
#   data_p3 <- read.csv(snapshot_world) %>%
#     rename(model=Model, scenario=Scenario, region=Region, variable=Variable, unit=Unit) %>%
...
#     filter(!is.na(value))
# 
#   data_ngfs_world <- data_p3 %>% filter(region=="World", period %in% seq(2005,2100,5))
#   data_ngfs_world$model <- as.character(data_ngfs_world$model)
# 
# data_ngfs_world <- data_ngfs_world %>%
...
#   mutate(model = ifelse(grepl("REMIND", model),    "REMIND-MAgPIE", model))
  ...
  # TODO: this could be replaced with new data/generic phase 2 data 
  load(file="C:/Users/pascalwe/Code/NGFS/iea_netzero_2021/data/ngfs_data_world_v2p2.RData")
  # Load SR 15, AR6 data
  load(file="C:/Users/pascalwe/Code/NGFS/iea_netzero_2021/data/IPCC_data_sr15_ar6.RData")
  #reduce size...
